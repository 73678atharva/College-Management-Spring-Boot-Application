...
public class AdminServiceImp implements AdminService {
	...
	public AdminDTO getCurrentAdminDetials(String adminUserName) {
		AdminDTO existingAdmin = AdminMapper.converAdminToAdminDTO(adminRepository.findAdminByUserName(adminUserName));
		if (existingAdmin == null)
			throw new CustomException("Admin with userName: " + adminUserName + " not Found: ", HttpStatus.NOT_FOUND);
		return existingAdmin;
	}

	@Override
	public Admin updateCurrentAdminDetails(Admin admin) {
		Admin updatedAdmin = adminRepository.findAdminByUserName(admin.getUser().getUsername());
		updatedAdmin.setFirstName(admin.getFirstName());
		updatedAdmin.setLastName(admin.getLastName());

		if (admin.getUser() != null) {
			User updatedUser = userRepository.findByUsername(admin.getUser().getUsername());
			updatedUser.setUsername(admin.getUser().getUsername());
			updatedUser.setPassword(bCryptPasswordEncoder.encode(admin.getUser().getPassword()));
			updatedUser.setRoles(String.join(",", Arrays.asList("USER", "ADMIN")));

			updatedAdmin.setUser(updatedUser);
		}
		Admin newUpadtedAdmin = adminRepository.save(updatedAdmin);
		if (newUpadtedAdmin == null)
			throw new CustomException("Admin updation failed", HttpStatus.NOT_MODIFIED);
		return newUpadtedAdmin;
	}
}
