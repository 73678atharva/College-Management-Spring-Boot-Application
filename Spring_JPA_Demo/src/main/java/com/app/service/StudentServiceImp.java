...
public class StudentServiceImp implements StudentService {
	...
	public StudentDTO updateCurrentStudentDetails(Student student) {
		Student existingStudent = studentRepository.findStudentByUserName(student.getUser().getUsername());

		existingStudent.setFirstName(student.getFirstName());
		existingStudent.setLastName(student.getLastName());

		List<Course> studentUpdatedCourses = new ArrayList<>();
		if (student.getCourses() != null) {
			for (Course course : student.getCourses()) {
				Course existingCourse = courseRepository.findByCourseName(course.getCourseName());
				studentUpdatedCourses.add(existingCourse);
			}
			existingStudent.setCourses(studentUpdatedCourses);
		}
		
		if(student.getUser() != null) {
			User existingUser = userRepository.findByUsername(student.getUser().getUsername());
			UserDTO existingUserDTO = UserMapper.convertUserToUserDTO(existingUser);
			
			existingUserDTO.setId(existingUser.getId());
			existingUserDTO.setUsername(student.getUser().getUsername());
		
			// checking if the password is updated then only we have to encode it again
			String incomingPassword = student.getUser().getPassword();
			if(!incomingPassword.isEmpty() && incomingPassword != null) {
				if(!bCryptPasswordEncoder.matches(incomingPassword, existingStudent.getUser().getPassword()))
					existingUserDTO.setPassword(bCryptPasswordEncoder.encode(incomingPassword));
				else
					existingUserDTO.setPassword(existingUser.getPassword());
			}
			existingStudent.setUser(UserMapper.convertUserDTOtoUser(existingUserDTO, existingUser.getRoles()));
		}
		
		 Student savedStudent = studentRepository.save(existingStudent);
		 if(savedStudent == null)
			 throw new CustomException("Student not updated successfully!!!", HttpStatus.NOT_MODIFIED);
